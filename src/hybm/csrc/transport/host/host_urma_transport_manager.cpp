...
using device::DeserializeExportDesc;
using device::UrmaExportDesc;
...
Result SerializeUrmaPrivateData(const UrmaEndpointDesc &desc, TransportPrivateData &data)
{
    UrmaPrivateDataDesc header{};
    header.payloadLen = static_cast<uint16_t>(sizeof(UrmaEndpointDesc));
    auto *raw = reinterpret_cast<uint8_t *>(data.key.keys);
    std::memcpy(raw, &header, sizeof(UrmaPrivateDataDesc));
    std::memcpy(raw + sizeof(UrmaPrivateDataDesc), &desc, sizeof(UrmaEndpointDesc));
    return BM_OK;
}

Result ParseUrmaPrivateData(const TransportPrivateData &privateData, UrmaEndpointDesc &outDesc)
{
    ...
    if (header.magic != URMA_PRIVATE_DATA_MAGIC) {
        BM_LOG_ERROR("host_urma ParseUrmaPrivateData invalid magic: 0x" << std::hex << header.magic << " expected: 0x"
                                                                        << URMA_PRIVATE_DATA_MAGIC);
        return BM_INVALID_PARAM;
    }
    if (header.version != URMA_PRIVATE_DATA_VERSION) {
        BM_LOG_ERROR("host_urma ParseUrmaPrivateData unsupported version: "
                     << header.version << " expected: " << URMA_PRIVATE_DATA_VERSION);
        return BM_INVALID_PARAM;
    }
    if (header.payloadLen != sizeof(UrmaEndpointDesc)) {
        BM_LOG_ERROR("host_urma ParseUrmaPrivateData payloadLen mismatch: "
                     << header.payloadLen << " expected: " << sizeof(UrmaEndpointDesc));
        return BM_INVALID_PARAM;
    }
    if (headerSize + header.payloadLen > sizeof(privateData.key.keys)) {
        BM_LOG_ERROR("host_urma ParseUrmaPrivateData payload exceeds key capacity, payloadLen: " << header.payloadLen);
        return BM_INVALID_PARAM;
    }
...
{
    if (memKey.keys[0] != URMA_EXPORT_DESC_MAGIC || memKey.keys[1] == 0) {
    ...
    parsed.payload = reinterpret_cast<const uint8_t *>(&memKey.keys[DEVICE_URMA_EXPORT_KEY_HEADER_SLOTS]);
    ...
    if (desc.magic != URMA_EXPORT_DESC_MAGIC || desc.version != URMA_EXPORT_DESC_VERSION ||
        ...
        desc.hcommDescLen == 0 || !device::GetRangeEnd(exportMem, exportEnd) ||
        totalDescLen > DEVICE_URMA_EXPORT_KEY_DATA_BYTES) {
...
Result HostUrmaTransportManager::BuildLocalHostEndpointDescLocked(UrmaEndpointDesc &endpoint) const
{
    ...
    endpoint.locType = ENDPOINT_LOC_TYPE_HOST;
    endpoint.hostId = rankId_;
    return BM_OK;
}
...
Result HostUrmaTransportManager::QueryMemoryKey(uint64_t addr, TransportMemoryKey &key)
{
    ...
    constexpr uint32_t maxPayloadBytes = (KEY_SIZE * 6 - DEVICE_URMA_EXPORT_KEY_HEADER_SLOTS) * sizeof(uint64_t);
...
Result HostUrmaTransportManager::Prepare(const HybmTransPrepareOptions &options)
{
    ...
    for (const auto &[peerRank, peerInfo] : options.options) {
        ...
        if (isNew) {
            ...
            auto ret = ParseUrmaPrivateData(peerInfo.privateData, peerEndpoint);
            if (ret != BM_OK) {
...
{
    EndpointDesc hcommEndpoint = ToHcommEndpointDesc(state.endpointDesc);
...
Result HostUrmaTransportManager::PreparePeerMemoryKeysLocked(uint32_t peerRank,
                                                             ...
                                                             RemoteRankState &state)
{
    if (state.endpointDesc.locType == ENDPOINT_LOC_TYPE_DEVICE) {
        return ValidateDevicePeerMemoryKeysLocked(peerRank, memKeys);
    }
    if (state.endpointDesc.locType == ENDPOINT_LOC_TYPE_HOST) {
    ...
    BM_LOG_ERROR("Unsupported peer endpoint location, rankId: " << rankId_ << " peerRank: " << peerRank
                                                                << " locType: " << state.endpointDesc.locType);
    return BM_INVALID_PARAM;
}
                                                           ...
                                                           RemoteRankState &state)
{
    ...
    for (const auto &memKey : memKeys) {
        if (memKey.keys[0] != URMA_EXPORT_DESC_MAGIC) {
        ...
        const uint8_t *payload =
            reinterpret_cast<const uint8_t *>(&memKey.keys[DEVICE_URMA_EXPORT_KEY_HEADER_SLOTS]);
        UrmaExportDesc exportDesc{};
    ...
    bool needFallback = false;
    {
        ...
        for (const auto &[peerRank, peerInfo] : options.options) {
            ...
            if (peerInfo.privateData.ip[0] != '\0') {
                ...
                auto ret = ParseUrmaPrivateData(peerInfo.privateData, newDesc);
                if (ret != BM_OK) {
                    return ret;
                }
...
const TransportPrivateData HostUrmaTransportManager::GetPrivateData() const
{
    ...
    auto ret = SerializeUrmaPrivateData(localEndpointDesc_, data);
    if (ret != BM_OK) {
