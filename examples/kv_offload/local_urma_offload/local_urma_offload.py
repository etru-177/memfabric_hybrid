...
def _rank_main(rank_id: int, device_id: int, world_size: int, port: int, sync: mp.Barrier):
    ...
    src_ptrs = torch.tensor(data['cpu']['key_ptrs'] + data['cpu']['value_ptrs'], dtype=torch.int64)
    dst_ptrs = torch.tensor(data['npu']['key_ptrs'] + data['npu']['value_ptrs'], dtype=torch.int64)
    len_ptrs = torch.tensor(data['len']['keys'] + data['len']['values'], dtype=torch.int32)
    size_ptr = torch.tensor(size, dtype=torch.int32)
...
def _parse_world_size() -> tuple:
    parser = argparse.ArgumentParser(
        description="local_urma_offload: local URMA pool KV offload (host+device entity pair)")
    parser.add_argument(
        "--world_size",
        type=int,
...
def main():
    ...
    print("local_urma_offload: all ranks OK", flush=True)
